...
from wdoo.exceptions import CacheMiss
...
class Field(MetaField('DummyField', (object,), {})):
    ...
    :param str group_operator: aggregate function used by :meth:`~wdoo.models.Model.read_group`
    ...
    def groupable(self):
        """
        Return whether the field may be used for grouping in :meth:`~wdoo.models.BaseModel.read_group`.
        """
    ...
    def mapped(self, records):
        ...
        over a simple call to `~wdoo.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~wdoo.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~wdoo.fields.Float.round()` to round a float with the given precision.
    :func:`~wdoo.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~wdoo.fields.Float.compare()` to compare two floats at the given precision.
...
class Binary(Field):
    ...
    def convert_to_column(self, value, record, values=None, validate=True):
        # The base64 string may be provided as a
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`wdoo.tools.image.ImageProcess` for maximum image resolution (default: ``45e6``).
...
class Many2one(_Relational):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~wdoo.models.Model._check_company`. Add a default company
...
class Command(enum.IntEnum):
    """
    :class:`~wdoo.fields.One2many` and :class:`~wdoo.fields.Many2many` fields
    ...
    def create(cls, values: dict):
        ...
        In case of a :class:`~wdoo.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~wdoo.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int):
        ...
        In case of a :class:`~wdoo.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int):
        ...
        In case of a :class:`~wdoo.fields.One2many` relation, the given record
