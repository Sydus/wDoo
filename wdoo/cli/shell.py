...
class Shell(Command):
    """Start wdoo in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        wdoo.cli.server.report_configuration()
        wdoo.service.server.start(preload=[], stop=True)
        ...
        with wdoo.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = wdoo.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = wdoo.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
