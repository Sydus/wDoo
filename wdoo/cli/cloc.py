...
from wdoo.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    wdoo cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        wdoo-bin cloc -p module_path

    Or by providing the name of a database:

        wdoo-bin cloc --addons-path=dirs -d database
