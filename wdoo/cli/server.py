...
__author__ = wdoo.release.author
__version__ = wdoo.release.version

# Also use the `wdoo` logger for the main script.
_logger = logging.getLogger('wdoo')
...
def report_configuration():
    ...
    """
    config = wdoo.tools.config
    _logger.info("wdoo version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', wdoo.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = wdoo.tools.config
    if not wdoo.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = wdoo.modules.registry.Registry.new(dbname)
        with wdoo.api.Environment.manage():
            with registry.cursor() as cr:
                wdoo.tools.trans_export(config["language"],
...
def main(args):
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                wdoo.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except wdoo.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        wdoo.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = wdoo.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the wdoo server (default command)"""
    def run(self, args):
        main(args)
