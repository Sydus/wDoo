...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('wdoo.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='wdoo')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='wdoo.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'wdoo.http.rpc.request:INFO',
    'wdoo.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['wdoo:DEBUG', 'wdoo.sql_db:INFO', 'wdoo.http.rpc:DEBUG'],
    'debug_rpc': ['wdoo:DEBUG', 'wdoo.sql_db:INFO', 'wdoo.http.rpc.request:DEBUG'],
    'debug': ['wdoo:DEBUG', 'wdoo.sql_db:INFO'],
    'debug_sql': ['wdoo.sql_db:DEBUG'],
    'info': [],
    'runbot': ['wdoo:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['wdoo:WARNING', 'werkzeug:WARNING'],
    'error': ['wdoo:ERROR', 'werkzeug:ERROR'],
    'critical': ['wdoo:CRITICAL', 'werkzeug:CRITICAL'],
}
