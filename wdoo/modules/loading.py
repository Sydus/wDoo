...
import wdoo
import wdoo.modules.db
import wdoo.modules.graph
import wdoo.modules.migration
import wdoo.modules.registry
from .. import SUPERUSER_ID, api, tools
from .module import adapt_version, initialize_sys_path, load_wdoo_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('wdoo.tests')
...
def force_demo(cr):
    ...
    """
    graph = wdoo.modules.graph.Graph()
...
def load_module_graph(cr, graph, perform_checks=True,
    ...
    processed_modules = []
    loaded_modules = []
    registry = wdoo.registry(cr.dbname)
    migrations = wdoo.modules.migration.MigrationManager(cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = wdoo.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = wdoo.sql_db.sql_counter
        ...
        load_wdoo_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['wdoo.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = wdoo.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                ...
                tests_t0, tests_q0 = time.time(), wdoo.sql_db.sql_counter
                test_results = loader.run_suite(suite, module_name)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = wdoo.sql_db.sql_counter - tests_q0
        ...
        extra_queries = wdoo.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   wdoo.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not wdoo.modules.db.is_initialized(cr):
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['wdoo.addons.%s' % (pkg.name,)]
                ...
                registry = wdoo.modules.registry.Registry.new(
