...
import wdoo
import wdoo.tools as tools
import wdoo.release as release
from wdoo.tools import pycompat

MANIFEST_NAMES = ('__manifest__.py', '__wdoo__.py')
...
class AddonsHook(object):
    """ Makes modules accessible through wdoo.addons.* """
    ...
    def load_module(self, name):
        ...
        wdoo_module = sys.modules.get(name)
        if not wdoo_module:
            wdoo_module = importlib.import_module(name)

        sys.modules[name] = wdoo_module

        return wdoo_module

class wdooHook(object):
    """ Makes wdoo package also available as wdoo """
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `wdoo.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^wdoo.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("wdoo.addons.base.maintenance.migrations", "wdoo.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``wdoo.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook wdoo.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in wdoo.addons.__path__:
        wdoo.addons.__path__.append(dd)

    # hook wdoo.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in wdoo.addons.__path__:
            wdoo.addons.__path__.append(ad)

    # hook wdoo.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in wdoo.addons.__path__ and os.path.isdir(base_path):
        wdoo.addons.__path__.append(base_path)

    # hook wdoo.upgrade on upgrade-path
    from wdoo import upgrade
    ...
    # create decrecated module alias from wdoo.addons.base.maintenance.migrations to wdoo.upgrade
    spec = importlib.machinery.ModuleSpec("wdoo.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["wdoo.addons.base.maintenance"] = maintenance_pkg
    sys.modules["wdoo.addons.base.maintenance.migrations"] = upgrade


    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, wdooHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in wdoo.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = wdoo.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in wdoo.addons.__path__:
...
def load_wdoo_module(module_name):
    ...
    try:
        __import__('wdoo.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['wdoo.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in wdoo.addons.__path__:
