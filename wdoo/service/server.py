...
import wdoo
from wdoo.modules import get_modules
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'wdoo.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The wdoo_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("wdoo_MAX_HTTP_THREADS")
    ...
    # is included in all Python 3 releases installed on all operating systems supported by wdoo.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(wdoo, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in wdoo.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="wdoo.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from wdoo.addons.base.models.ir_cron import ir_cron
        conn = wdoo.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = wdoo.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(wdoo.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="wdoo.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="wdoo.service.httpd")
        t.daemon = True
        t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = wdoo.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('wdoo: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(wdoo.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The wdoo_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("wdoo_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = wdoo.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('wdoo.netsvc.rpc.request')
    ...
    def start(self):
        ...
        dbconn = wdoo.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', } | set(wdoo.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            wdoo.modules.module.load_wdoo_module(m)
        except Exception:
            msg = ''
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute wdoo-server process with (nearly) the same arguments"""
    if wdoo.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from wdoo.tests.common import wdooSuite
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = wdoo.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             wdoo.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the wdoo http server and cron processor.
    """
    global server
    ...
    if wdoo.evented:
        server = GeventServer(wdoo.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(wdoo.service.wsgi_server.application)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which wdoo is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(wdoo.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not wdoo.evented:
        if inotify:
