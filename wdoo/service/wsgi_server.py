...
# User code must use the exceptions defined in ``wdoo.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, wdoo.exceptions.RedirectWarning):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, wdoo.exceptions.AccessError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, wdoo.exceptions.AccessDenied):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, wdoo.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = wdoo.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, wdoo.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(wdoo.tools.exception_to_unicode(e), formatted_info)
...
def application_unproxied(environ, start_response):
    ...
    # wdoo.service.web_services.objects_proxy.dispatch().
    ...
    result = wdoo.http.root(environ, start_response)
