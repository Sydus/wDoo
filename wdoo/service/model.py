...
import wdoo
from wdoo.exceptions import UserError, ValidationError
from wdoo.models import check_method_name
from wdoo.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = wdoo.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(wdoo.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if wdoo.registry(dbname)._init and not wdoo.tools.config['test_enable']:
                    raise wdoo.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    cr.reset()
    recs = wdoo.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = wdoo.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with wdoo.registry(db).cursor() as cr:
