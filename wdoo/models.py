    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('wdoo.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'wdoo.addons'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for wdoo models.

    wdoo models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~wdoo.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~wdoo.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~wdoo.models.AbstractModel`.
    """
    ...
    _module = None              #: the model's module (in the wdoo sense)
    ...
    .. warning::
      ...
      :attr:`~wdoo.models.Model._inherits`-ed models, the inherited field will
    ...
    def _compute_concurrency_field(self):
        ...
        if self._log_access:
            ...
            self[fname] = wdoo.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], wdoo.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], wdoo.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from wdoo.fields import Char, Integer
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False):
        ...
        result = self._fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar)
    ...
    def _read_group_fill_temporal(self, data, groupby, aggregated_fields, annotated_groupbys,
        ...
        if fill_from:
            fill_from = date_utils.start_of(wdoo.fields.Datetime.to_datetime(fill_from), granularity)
            if tz:
                fill_from = tz.localize(fill_from)
        elif existing_from:
            fill_from = existing_from
        if fill_to:
            fill_to = date_utils.start_of(wdoo.fields.Datetime.to_datetime(fill_to), granularity)
            if tz:
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~wdoo.addons.base.models.res_company`).

        For :class:`~wdoo.addons.base.models.res_users` relational fields,
    ...
    def unlink(self):
        ...
        from wdoo.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~wdoo.fields.Integer`,
          :class:`~wdoo.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~wdoo.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~wdoo.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~wdoo.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~wdoo.fields.Date` and
              :class:`~wdoo.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`wdoo.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`wdoo.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _wdoo/models/relationals/format:

          The expected value of a :class:`~wdoo.fields.One2many` or
          :class:`~wdoo.fields.Many2many` relational field is a list of
          :class:`~wdoo.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~wdoo.fields.Command.create`,
          :meth:`~wdoo.fields.Command.update`,
          :meth:`~wdoo.fields.Command.delete`,
          :meth:`~wdoo.fields.Command.unlink`,
          :meth:`~wdoo.fields.Command.link`,
          :meth:`~wdoo.fields.Command.clear`, and
          :meth:`~wdoo.fields.Command.set`.
          """
        if not self:
            return True
    ...
    def ensure_one(self):
        ...
        :raise wdoo.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~wdoo.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~wdoo.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=lambda item: item[0].write_sequence):
            ...
            if field.relational:
                ...
                for invf in self.pool.field_inverses[field]:
                    ...
                    # /home/dle/src/wdoo/master-nochange-fp/wdoo/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`wdoo.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted wdoo models.

    wdoo models are created by inheriting from this class::
