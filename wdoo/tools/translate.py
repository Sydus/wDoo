...
WEB_TRANSLATION_COMMENT = "wdoo-web"
...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return wdoo.sql_db.db_connect(db_name)
...
def TranslationFileReader(source, fileformat='po'):
    """ Iterate over translation file to return wdoo translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return wdoo translation entries """
    def __init__(self, source):
...
def _extract_translatable_qweb_terms(element, callback):
    ...
    for el in element:
        ...
        if (el.tag.lower() not in SKIPPED_ELEMENTS
            ...
            # https://github.com/wdoo/owl/blob/master/doc/reference/component.md#composition
...
class TranslationModuleReader:
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._lang = lang
        self.env = wdoo.api.Environment(cr, wdoo.SUPERUSER_ID, {})
        self._to_translate = []
        self._path_list = [(path, True) for path in wdoo.addons.__path__]
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'wdoo.tools.translate:babel_extract_qweb',
                                                  extra_comments=[WEB_TRANSLATION_COMMENT])
