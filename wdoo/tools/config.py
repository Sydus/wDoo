...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("--load", dest="server_wide_modules", help="Comma-separated list of server-wide modules.", my_default='base')

        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'wdoo_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "wdoo.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="wdoo.http.rpc.request:DEBUG", help='shortcut for --log-handler=wdoo.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="wdoo.http.rpc.response:DEBUG", help='shortcut for --log-handler=wdoo.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="wdoo.http:DEBUG", help='shortcut for --log-handler=wdoo.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="wdoo.sql_db:DEBUG", help='shortcut for --log-handler=wdoo.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate wDoo to another language. "
    ...
    def parse_config(self, args=None):
        ...
        arguments.
        This method initializes wDoo.tools.config and wdoo.conf (the
        ...
        Typical usage of this method:
            wdDoo.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        wdoo.netsvc.init_logger()
        wdoo.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        die(opt.config and not os.access(opt.config, os.R_OK),
        ...
        # TODO use appdirs
        rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'wdoo.conf')

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('WDOO_RC') or rcfilepath)
        self.load()
        ...
        keys = ['http_interface', 'http_port', 'longpolling_port', 'http_enable',
                ...
                'db_port', 'db_template', 'logfile', 'pidfile',                
                'db_maxconn', 'addons_path', 'upgrade_path',
                'syslog', 'screencasts', 'screenshots',
        ...
        keys = [
            ...
            'dev_mode', 'shell_interface', 'load_language',
            'stop_after_init', 'http_enable', 'syslog',
            'list_db', 'proxy_mode',
            'test_file', 'test_tags',
    ...
    def _is_addons_path(self, path):
        from wdoo.modules.module import MANIFEST_NAMES
