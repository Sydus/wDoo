...
from wdoo import SUPERUSER_ID, api
from wdoo.exceptions import ValidationError
...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  Command=wdoo.fields.Command,
                  time=time,
                  DateTime=datetime,
                  datetime=datetime,
                  timedelta=timedelta,
                  relativedelta=relativedelta,
                  version=wdoo.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('wdoo.tools.convert.init').error(
        ...
        # invoke method
        return wdoo.api.call_kw(model, method_name, args, kwargs)
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        for group in rec.get('groups', '').split(','):
            if group.startswith('-'):
                ...
                groups.append(wdoo.Command.unlink(group_id))
            elif group:
                group_id = self.id_get(group)
                groups.append(wdoo.Command.link(group_id))
        if groups:
    ...
    def _tag_record(self, rec):
        ...
        for field in rec.findall('./field'):
            ...
            if f_search:
                ...
                if (f_name in _fields) and _fields[f_name].type == 'many2many':
                    f_val = [wdoo.Command.set([x[f_use] for x in s])]
                elif len(s):
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler wdoo.tools.convert:DEBUG" for complete traceback
    ...
    def __init__(self, cr, module, idref, mode, noupdate=False, xml_filename=None):
        self.mode = mode
        self.module = module
        self.envs = [wdoo.api.Environment(cr, SUPERUSER_ID, {})]
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <wdoo>, <wdoo> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['wdoo', 'data', 'wdoo']
...
def convert_csv_import(cr, module, fname, csvcontent, idref=None, mode='init',
    ...
    env = wdoo.api.Environment(cr, SUPERUSER_ID, context)
