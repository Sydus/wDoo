...
import wdoo
from wdoo.tools.config import config
...
STANDARD_MODULES = ['web', 'theme_common', 'base']
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in wdoo.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [wdoo.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(wdoo.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        for a in env['ir.actions.server'].browse(data.keys()):
            assert [a.id][0], "Missing action server %s" % a.id
            self.book(
                data[a.id][0],
        ...
        query = r"""
                SELECT f.id, f.name, min(m.name), array_agg(d.module)
                  FROM ir_model_fields AS f
             LEFT JOIN ir_model_data AS d ON (d.res_id = f.id AND d.model = 'ir.model.fields')
             LEFT JOIN ir_module_module AS m ON m.name = d.module AND m.imported = TRUE AND m.state = 'installed'
        ...
        data = {r[0]: (r[2], r[3]) for r in all_data}
        for f in env['ir.model.fields'].browse(data.keys()):
            assert data[f.id][0], "Missing field %s" % f.id
            self.book(
                data[f.id][0],
    ...
    def count_database(self, database):
        registry = wdoo.registry(config['db_name'])
        with registry.cursor() as cr:
            uid = wdoo.SUPERUSER_ID
            env = wdoo.api.Environment(cr, uid, {})
            self.count_env(env)
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="wdoo cloc", lines="Line", other="Other", code="Code")
        s += hr
