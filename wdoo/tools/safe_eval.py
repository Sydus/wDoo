...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    except wdoo.exceptions.UserError:
        raise
    except wdoo.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except wdoo.http.AuthenticationError:
        raise
...
`wdoo.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.
Pre-wrapped modules are provided as attributes of `wdoo.tools.safe_eval`.
""")
    return d
