"""
This code is what let us use ES6-style modules in wdoo.
Classic wdoo modules are composed of a top-level :samp:`wdoo.define({name},{body_function})` call.
This processor will take files starting with an `@wdoo-module` annotation (in a comment) and convert them to classic modules.
If any file has the /** wdoo-module */ on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual wdoo one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom wdoo modules.
    ...
    steps = [
        ...
        partial(wrap_with_wdoo_define, module_path),
    ]
    for s in steps:
        content = s(content)
...
def url_to_module_path(url):
    """
    wdoo modules each have a name. (wdoo.define("<the name>", async function (require) {...});
...
def wrap_with_wdoo_define(module_path, content):
    """
    Wraps the current content (source code) with the wdoo.define call.
    Should logically be called once all other operations have been performed.
    """
    return f"""wdoo.define({module_path!r}, async function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
def get_aliased_wdoo_define_content(module_path, content):
    ...
    .. code-block:: javascript
        ...
        /** @wdoo-module */
        // after
        /** @wdoo-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @wdoo-module */
        // after
        /** @wdoo-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nwdoo.define(`%s`, async function(require) {
                        return require('%s');
                        });\n""" % (alias, module_path)
            else:
                return """\nwdoo.define(`%s`, async function(require) {
