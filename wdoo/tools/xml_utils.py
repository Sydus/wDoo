...
class wdoo_resolver(etree.Resolver):
    """wdoo specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None):
    ...
    :param env (wdoo.api.Environment): If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(wdoo_resolver(env))
