...
from wdoo.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if wdoo.tools.config['db_host']:
        env['PGHOST'] = wdoo.tools.config['db_host']
    if wdoo.tools.config['db_port']:
        env['PGPORT'] = str(wdoo.tools.config['db_port'])
    if wdoo.tools.config['db_user']:
        env['PGUSER'] = wdoo.tools.config['db_user']
    if wdoo.tools.config['db_password']:
        env['PGPASSWORD'] = wdoo.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',)):
    ...
    addons_paths = wdoo.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None):
    ...
    >>> with file_open('/opt/wdoo/addons/hr/static/description/icon.png', 'rb') as f:
    ...     contents = f.read()
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = wdoo.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        @mute_logger('wdoo.plic.ploc')
        def do_stuff():
            blahblah()
        with mute_logger('wdoo.foo.bar'):
            do_suff()
    """
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = wdoo.fields.Datetime.from_string(value)
            value = wdoo.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = wdoo.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = wdoo.fields.Datetime.from_string(value)
    else:
        timestamp = value
