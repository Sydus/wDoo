...
# *broken*
NO_POSTMORTEM = (wdoo.exceptions.AccessDenied,
                 wdoo.exceptions.UserError,
                 wdoo.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                wdoo.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = wdoo.service.common.dispatch
        elif service_name == 'db':
            dispatch = wdoo.service.db.dispatch
        elif service_name == 'object':
            dispatch = wdoo.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                wdoo.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                wdoo.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(wdoo.tools.exception_to_unicode(e))
        wdoo.tools.debugger.post_mortem(wdoo.tools.config, sys.exc_info())
        raise


class WebRequest(object):
    """ Parent class for all wdoo Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in wdoo.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~wdoo.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~wdoo.api.Environment` bound to current request. """
        if self._env is None:
            self._env = wdoo.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            wdoo.tools.debugger.post_mortem(
                wdoo.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        wdoo implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <wdoo.http.WebRequest.csrf_token`, the
          :data:`~wdoo.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          wdoo) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (wdoo.exceptions.UserError,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "wdoo Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "wdoo Session Expired"
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["wdoo", "addons"]:
            module = ""
        else:
...
class EndPoint(object):
    ...
    # When wdoo clears the routing map, new EndPoint objects are created,
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('wdoo.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = wdoo.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = wdoo.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or wdoo.SUPERUSER_ID)
...
class Root(object):
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in wdoo.http._generate_routing_rules([''] + wdoo.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        for addons_path in wdoo.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request_manager:
                ...
                if db:
                    try:
                        wdoo.registry(db).check_signaling()
                        with wdoo.tools.mute_logger('wdoo.sql_db'):
...
def db_list(force=False, httprequest=None):
    try:
        dbs = wdoo.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if wdoo.tools.config['dbfilter']:
        ...
        exposed_dbs = set(db.strip() for db in wdoo.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = wdoo.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = wdoo.tools.ustr(filename)
