...
import wdoo
import wdoo.modules.registry
from wdoo.api import call_kw
from wdoo.addons.base.models.qweb import QWeb
from wdoo.modules import get_resource_path, module
from wdoo.tools import html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property, float_repr, osutil
from wdoo.tools.mimetypes import guess_mimetype
from wdoo.tools.translate import _
from wdoo.tools.misc import str2bool, xlsxwriter, file_open, file_path
from wdoo.tools.safe_eval import safe_eval, time
from wdoo import http
from wdoo.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception
from wdoo.exceptions import AccessError, UserError, AccessDenied
from wdoo.models import check_method_name
from wdoo.service import db, security
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = wdoo.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except wdoo.exceptions.AccessDenied:
        ...
        if request.httprequest.method == 'POST':
            ...
            except wdoo.exceptions.AccessDenied as e:
                request.uid = old_uid
                if e.args == wdoo.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = wdoo.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def qweb(self, unique, mods=None, db=None, bundle=None):
        ...
        if not request.db and mods is None:
            mods = wdoo.conf.server_wide_modules or []
    ...
    def translations(self, unique, mods=None, lang=None):
        ...
        if mods:
            ...
            mods = list(request.env.registry._init_modules) + (wdoo.conf.server_wide_modules or [])
    ...
    def version_info(self):
        return wdoo.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = wdoo.tools.config.verify_admin_password('admin')
        d['list_db'] = wdoo.tools.config['list_db']
        d['countries'] = list(pytz.country_names.items())
        d['langs'] = wdoo.service.db.exp_list_lang()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = wdoo.service.db.list_db_incompatible(d['databases'])
        except wdoo.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = wdoo.tools.config.verify_admin_password('admin')
        ...
        try:
            ...
            tz = None
            if post.get('country'):
                if len(pytz.country_timezones[post['country']]) == 1:
                    tz = pytz.country_timezones[post['country']][0]
    
            dispatch_rpc('db', 'create_database', [master_pwd, name, bool(post.get('demo')), lang, password, post['login'], tz])
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            wdoo.service.db.check_super(master_pwd)
            ...
            dump_stream = wdoo.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def account(self):
        ...
        return 'https://accounts.wdoo.com/oauth2/auth?' + url_encode(params)
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: wdoo.tools.ustr(field[1].get('string', '').lower()))
