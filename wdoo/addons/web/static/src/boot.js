...
(function () {
    ...
    var wdoo = window.wdoo;
    var debug = wdoo.debug;
    ...
    wdoo.testing = typeof QUnit === "object";
    wdoo.remainingJobs = jobs;
    wdoo.__DEBUG__ = {
    ...
    wdoo.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === "string" ? args.shift() : "__wdoo_job" + jobUID++;
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: wdoo.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name,
                };
                if (jobs[k].error) {
                ...
                var deps = wdoo.__DEBUG__.getDependencies(job.name);
            ...
            missing = wdoo.__DEBUG__.getMissingJobs();
            failed = wdoo.__DEBUG__.getFailedJobs();
    ...
    wdoo.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec)
                        .then(function (data) {
                            ...
                            resolve();
                            wdoo.processJobs(jobs, services);
                        })
