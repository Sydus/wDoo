...
(function (exports) {
    ...
    const BLACKLISTED_MENUS = [
        ...
        "payment_wdoo.menu_adyen_account",
        "payment_wdoo.root_adyen_menu",
    ];

    const { isEnterprise } = wdoo.info;
    ...
    function ensureSetup() {
        ...
        setupDone = true;
        const env = wdoo.__WOWL_DEBUG__.root.env;
        env.bus.on("ACTION_MANAGER:UI-UPDATED", null, () => {
            actionCount++;
        });

        const AbstractController = wdoo.__DEBUG__.services["web.AbstractController"];
        ...
        const { patch } = wdoo.__DEBUG__.services["@web/core/utils/patch"];
        const { WithSearch } = wdoo.__DEBUG__.services["@web/search/with_search/with_search"];
        ...
        // This test file is not respecting Wdoo module dependencies.
        // The following module might not be loaded (eg. if mail is not installed).
        const DiscussWidgetModule = wdoo.__DEBUG__.services["@mail/widgets/discuss/discuss"];
    ...
    async function _clickEverywhere(xmlId) {
        ...
        console.log(`Wdoo flavor: ${isEnterprise ? "Enterprise" : "Community"}`);
