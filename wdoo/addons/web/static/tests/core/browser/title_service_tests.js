...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Wdoo" });
    assert.strictEqual(env.services.title.current, "Wdoo");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Wdoo", chat: null });
    assert.strictEqual(env.services.title.current, "Wdoo");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Wdoo - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Wdoo", action: "Import" });
    assert.strictEqual(env.services.title.current, "Wdoo - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Wdoo", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Wdoo - Import"); // parts is a copy!
});
