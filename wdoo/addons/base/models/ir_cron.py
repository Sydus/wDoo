...
import wdoo
from wdoo import api, fields, models, _
from wdoo.exceptions import UserError

_logger = logging.getLogger(__name__)

BASE_VERSION = wdoo.modules.load_information_from_description_file('base')['version']
...
class ir_cron(models.Model):
    ...
    def create(self, values):
        ...
        if os.getenv('wdoo_NOTIFY_CRON_CHANGES'):
        ...
        try:
            ...
            with db.cursor() as cron_cr:
                ...
                while True:
                    ...
                    registry = wdoo.registry(db_name)
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        wdoo.modules.reset_modules_state(cr.dbname)
    ...
    def _callback(self, cron_name, server_action_id, job_id):
        ...
        try:
            ...
            wdoo.netsvc.log(_logger, logging.DEBUG, 'cron.object.execute', (self._cr.dbname, self._uid, '*', cron_name, server_action_id), depth=log_depth)
            start_time = False
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('wdoo_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('wdoo_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)

    def _notifydb(self):
        """ Wake up the cron workers
        The wdoo_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with wdoo.sql_db.db_connect('postgres').cursor() as cr:
