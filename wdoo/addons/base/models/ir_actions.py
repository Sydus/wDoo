...
from wdoo.exceptions import MissingError, UserError, ValidationError, AccessError
from wdoo.osv import expression
from wdoo.tools.safe_eval import safe_eval, test_python_expr
from wdoo.tools.float_utils import float_compare
...
class IrActionsServer(models.Model):
    ...
    Since wdoo 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: wdoo Environment on which the action is triggered
#  - model: wdoo Model of the record on which the action is triggered; is a void recordset
#  - record: record on which the action is triggered; may be void
#  - records: recordset of all records on which the action is triggered in multi-mode; may be void
#  - time, datetime, dateutil, timezone: useful Python libraries
#  - float_compare: wdoo function to compare floats based on specific precisions
