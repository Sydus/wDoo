...
from wdoo import api, fields, models, tools, SUPERUSER_ID, _, Command
from wdoo.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from wdoo.exceptions import AccessDenied, AccessError, UserError, ValidationError
from wdoo.http import request
from wdoo.modules.module import get_module_resource
from wdoo.osv import expression
from wdoo.service.db import check_super
...
class Users(models.Model):
    ...
    def _unlink_except_superuser(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by wdoo (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: wdoo.exceptions.AccessDenied when old password is wrong
        :raise: wdoo.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your wdoo is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "wdoo in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.wdoo.com/documentation/15.0/administration/install/deploy.html#https for details.",
                    source
                )
