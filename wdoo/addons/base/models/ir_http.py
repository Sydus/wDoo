...
import wdoo
from wdoo import api, http, models, tools, SUPERUSER_ID
from wdoo.exceptions import AccessDenied, AccessError, MissingError
from wdoo.http import request, content_disposition, Response
from wdoo.tools import consteq, pycompat
from wdoo.tools.mimetypes import guess_mimetype
from wdoo.modules.module import get_resource_path, get_module_path

from wdoo.http import ALLOWED_DEBUG_MODES
from wdoo.tools.misc import str2bool
...
class IrHttp(models.AbstractModel):
    ...
    def _handle_exception(cls, exception):
        ...
        if (isinstance(exception, werkzeug.exceptions.HTTPException) and exception.code == 404) or \
           (isinstance(exception, wdoo.exceptions.AccessError)):
    ...
    def routing_map(cls, key=None):
        ...
        if key not in cls._routing_map:
            ...
            installed = request.registry._init_modules | set(wdoo.conf.server_wide_modules)
            if tools.config['test_enable'] and wdoo.modules.module.current_test:
                installed.add(wdoo.modules.module.current_test)
