...
from wdoo.modules import get_module_resource
from wdoo.tests.common import TransactionCase
from wdoo.addons.base.models.qweb import QWebException
from wdoo.tools import misc, mute_logger
...
class TestQWebNS(TransactionCase):
    ...
    def test_render_dynamic_xml_with_code_error(self):
        ...
        view1 = self.env['ir.ui.view'].create({
            ...
            'arch': u"""
                <t t-name="base.dummy">
                    <Invoice xmlns:od="http://wdoo.com/od">
...
class TestQWebBasic(TransactionCase):
    ...
    def test_attf_escaping_2(self):
        ...
        result = """
                <a href="/link/wdoo/sub">link</a>
                <a href="/link/wdoo/">link2</a>
            """
        values = {'url': 'wdoo', 'other': True}
        rendered = self.env['ir.qweb']._render(t.id, values)
        self.assertEqual(rendered.strip(), result.strip())
...
class TestQWebStaticXml(TransactionCase):
    ...
    @mute_logger('wdoo.addons.base.models.qweb') # tests t-raw which is deprecated
