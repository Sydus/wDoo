...
from wdoo.exceptions import AccessDenied, AccessError
from wdoo.http import _request_stack

import wdoo.tools
from wdoo.tests import common
from wdoo.service import common as auth, model
from wdoo.tools import DotDict
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, wdoo.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
