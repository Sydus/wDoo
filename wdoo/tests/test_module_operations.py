...
import wdoo
from wdoo.tools import topological_sort, unique
from wdoo.netsvc import init_logger
from wdoo.tests import standalone_tests
import wdoo.tests.loader

_logger = logging.getLogger('wdoo.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with wdoo.registry(db_name).cursor() as cr:
        env = wdoo.api.Environment(cr, wdoo.SUPERUSER_ID, {})
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of Wdoo modules")
    ...
    parser.add_argument("--data-dir", "-D", dest="data_dir", type=str,
        help="Directory where to store Wdoo data"
    )
    ...
    parser.add_argument("--addons-path", "-p", type=str,
        help="Comma-separated list of paths to directories containing extra Wdoo modules")
...
def test_full(args):
    ...
    with wdoo.registry(args.database).cursor() as cr:
        env = wdoo.api.Environment(cr, wdoo.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = wdoo.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        wdoo.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        wdoo.tools.config['addons_path'] = ','.join([args.addons_path, wdoo.tools.config['addons_path']])
        if args.data_dir:
            wdoo.tools.config['data_dir'] = args.data_dir
        wdoo.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('wdoo.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('wdoo.sql_db').setLevel(logging.CRITICAL)
