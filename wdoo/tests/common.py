# -*- coding: utf-8 -*-
"""
The module :mod:`wdoo.tests.common` provides unittest test cases and a few
...
import wdoo
from wdoo import api
from wdoo.models import BaseModel
from wdoo.exceptions import AccessError
from wdoo.modules.registry import Registry
from wdoo.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from wdoo.service import security
from wdoo.sql_db import BaseCursor, Cursor
from wdoo.tools import float_compare, single_email_re, profiler
from wdoo.tools.misc import find_in_path
from wdoo.tools.safe_eval import safe_eval
...
# The wdoo library is supposed already configured.
ADDONS_PATH = wdoo.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = wdoo.SUPERUSER_ID


def get_db_name():
    db = wdoo.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding wdoo module name.
    """
    def register(func):
        # register func by wdoo module name
        if func.__module__.startswith('wdoo.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most wdoo operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class WdooSuite(unittest.suite.TestSuite):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from wdoo.http import root
        if not root._loaded:
            root.load_addons()
            root._loaded = True
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('wdoo.addons.'):
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for wdoo-specific code. This class is abstract and
    ...
    longMessage = True      # more verbose error message by default: https://www.wdoo.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~wdoo.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('wdoo.sql_db.Cursor.execute', execute):
            with patch('wdoo.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/wdoo/addons/" in filename:
                        filename = filename.rsplit("/wdoo/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(self, records, expected_values):
        ...
        def _compare_candidate(record, candidate, field_names):
            ...
            for field_name in field_names:
                ...
                if field_type == 'float' and field.get_digits(record.env):
...
class TransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = wdoo.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, wdoo.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_wdoo')
        self.chrome_pid = None

        otc = wdoo.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way wdoo widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{wdoo.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True, head=False):
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, wdoo.tools.config['http_port'], url)
        if head:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('wdoo.service.http.request.')]
        ...
        if request_threads:
            ...
            wdoo.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        wdoo.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            wdoo.http.root.session_store.delete(self.session)

        self.session = session  = wdoo.http.root.session_store.new()
        session.db = db
        ...
        wdoo.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return "http://%s:%s" % (HOST, wdoo.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "wdoo.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "wdoo.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~wdoo.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~wdoo.fields.Many2many` fields, the field itself is a
    :class:`~wdoo.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~wdoo.fields.One2many` are reified as
    :class:`~wdoo.tests.common.O2MProxy`.

    Because the :class:`~wdoo.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~wdoo.tests.common.O2MProxy.new` and
    :meth:`~wdoo.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: wdoo.models.Model
    ...
    :type view: int | str | wdoo.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~wdoo.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~wdoo.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = wdoo.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = wdoo.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return wdoo.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return wdoo.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from wdoo.tests.common have a test_tags
