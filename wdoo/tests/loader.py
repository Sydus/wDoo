...
from .. import tools
from .common import TagsSelector, WdooSuite
from .runner import WdooTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('wdoo.addons', module)

    try:
        importlib.import_module('wdoo.upgrade.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('wdoo.upgrade', module)
...
def make_suite(module_names, position='at_install'):
    ...
    return WdooSuite(sorted(tests, key=lambda t: t.test_sequence))
